...
func part2() {
	...
	paths, cost := g.FindAllShortestPaths(startNode, endNode)
	if aoc.DebugMode {
		for _, path := range paths {
			gridCopy := grid.Copy()
			for _, node := range path {
				gridCopy.Data[node.Key.Point] = 'P'
			}
			aoc.Debug(gridCopy.String())
		}
		aoc.Debug(cost)
	}

	aoc.Println(t, "cost =", cost, ", len(paths) =", len(paths))

	uniqueLocations := make(map[aoc.Point]struct{})
	for _, path := range paths {
		for _, node := range path {
			uniqueLocations[node.Key.Point] = struct{}{}
		}
	}

	aoc.Println(t, len(uniqueLocations))
}
